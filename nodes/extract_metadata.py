...
def extract_metadata(state: MeetingState) -> MeetingState:
    ...
    prompt = """
    You are a meeting analyst. Extract the following metadata from the transcript below. Be precise and consistent.
    ...
    **date**: Extract the meeting date if explicitly mentioned or clearly implied (e.g., "today is March 5th", 
    a timestamp header). Format as YYYY-MM-DD. If absent, return null.
    ...
    metadata: MeetingMeta = structured_llm.invoke(messages)
